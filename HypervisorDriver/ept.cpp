...
InitializeEptp()
{
    ...
    //	Allocate EPT PML4
    //
    PEPT_PML4E EptPml4 = (PEPT_PML4E)ExAllocatePoolWithTag(NonPagedPool, PAGE_SIZE, POOLTAG);
    if (!EptPml4)
    {
    ...
    //
    PEPT_PDPTE EptPdpt = (PEPT_PDPTE)ExAllocatePoolWithTag(NonPagedPool, PAGE_SIZE, POOLTAG);
    if (!EptPdpt)
    {
    ...
    //
    PEPT_PDE EptPd = (PEPT_PDE)ExAllocatePoolWithTag(NonPagedPool, PAGE_SIZE, POOLTAG);
    ...
    //
    PEPT_PTE EptPt = (PEPT_PTE)ExAllocatePoolWithTag(NonPagedPool, PAGE_SIZE, POOLTAG);
    ...
    UINT64    GuestMemory     = (UINT64)ExAllocatePoolWithTag(NonPagedPool, PagesToAllocate * PAGE_SIZE, POOLTAG);
    if (GuestMemory == NULL)
    {
        ExFreePoolWithTag(EptPd, POOLTAG);
        ExFreePoolWithTag(EptPdpt, POOLTAG);
    ...
    for (size_t i = 0; i < PagesToAllocate; i++)
    {
        ...
        EptPt[i].Fields.PhysicalAddress    = (VirtualToPhysicalAddress(PVOID(GuestMemory + (i * PAGE_SIZE))) / PAGE_SIZE);
    ...
    DbgPrint("[*] Extended Page Table Pointer allocated at %llx", (ULONG64)EPTPointer);
